...
- (void)setIndex:(NSUInteger)value {
	if (value == NSNotFound) {
		...
		NSLog(@"Page: %d   index: %d  current_page: %d", page, index, current_page);
...
- (void)handleSingleTap:(CGPoint)touchPoint {
	NSLog(@"Single tap x: %f y: %f", touchPoint.x, touchPoint.y);
	if (touchPoint.y <= 60 || touchPoint.y >= self.frame.size.height - 50) {
		[photoBrowser performSelector:@selector(toggleControls) withObject:nil afterDelay:0.2];
	} else {
		[photoBrowser actionForSingleTap];
	}
}
- (void)handleDoubleTap:(CGPoint)touchPoint {
	...
	[photoBrowser actionForDoubleTap];
//	[photoBrowser actionForDoubleTap];
