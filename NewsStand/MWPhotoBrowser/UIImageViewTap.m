...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	switch (tapCount) {
		case 1:
			[self performSelector:@selector(handleSingleTap:) withObject:touch afterDelay:0.4];
			NSLog(@"Single tap in image view");
			break;
		case 2:
			[NSObject cancelPreviousPerformRequestsWithTarget:self];
			NSLog(@"Double tap in image view");
			[self performSelector:@selector(handleDoubleTap:) withObject:touch afterDelay:0.4];
			break;
		case 3:
			[NSObject cancelPreviousPerformRequestsWithTarget:self];
