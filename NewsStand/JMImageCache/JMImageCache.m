...
- (UIImage *) imageForURLOrThrowException:(NSString *)url delegate:(id<JMImageCacheDelegate>)d {
	if (!url) {
		return nil;
	}
	...
	if(returner) {
		...
		if(i) {
            @try {
                [self setImageOrThrowException:i forURL:url];
            }
            @catch (NSException *exception) {
                [[(NewsAnnotationView*) d newsAnnotation] setImageFailed:TRUE];
            }
...
- (void) setImage:(UIImage *)i forURL:(NSString *)url {
    ...
    @catch (NSException *e) {
        ...
        NSLog(@"Image exception thrown for url %@ in JMIImageCache: %@", url, e);
    }
}

- (void) setImageOrThrowException:(UIImage *)i forURL:(NSString *)url {
    @try {
        [super setObject:i forKey:url];

    } @catch (NSException *e) {
        [NSException raise:@"Invalid URL" format:@"TEST"];
    }
}
